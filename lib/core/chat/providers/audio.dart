...
final audioPlayerProvider = StateProvider.family<AudioPlayer, int>((ref, arg) {
  ...
  player.onPlayerStateChanged.listen((playStateEvent) async {
    if (playStateEvent == PlayerState.playing) {
      ref.read(proximitySubscriptionProvider.notifier).update((state) =>
          ProximitySensor.events
              .throttleTime(const Duration(milliseconds: 500))
              .listen((proximityEvent) async {
            if (player.state == PlayerState.playing) {
              ...
              ref
                  .read(proximitySubscriptionProvider.notifier)
                  .update((state) => null);
            }
      ...
      if (messageId != null) {
        ref
            .read(playedAudioMessageUuidsProvider(arg).notifier)
            .update((state) => {
                  ...state,
...
final playedAudioMessageUuidsProvider =
    StateProvider.autoDispose.family<Set<String>, int>((ref, arg) => {});

final audioMessagePlaySpeedProvider =
    StateProvider.family<double, int>((ref, arg) {
  var speed = 1.0;
...
final proximitySubscriptionProvider = StateProvider<StreamSubscription?>((ref) {
  ref.listenSelf((previous, next) async {
    try {
      ...
      if (kDebugMode)
        print('Error on cancel listening Proximity Sensor stream: $e');
    }
  });
  return null;
});
