...
/// 专业版法达盘 Widget，完全对应 FDAstrobaleSpecialView.java
class FDAstrolabeSpecialView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      size: size,
      painter: _FDAstrolabeSpecialPainter(
        houses: houses,
        planets: planets,
        phases: phases,
...
/// FDAstrolabeSpecialView 的私有 Painter
class _FDAstrolabeSpecialPainter extends CustomPainter {
  final List<Houses> houses;
  final List<Planets> planets;
  final List<Phase> phases;
  final double currentAge;
  ...
  void _calculateHousesPoints() {
    ...
    for (int i = 0; i < houses.length; i++) {
      ...
      if (i == houses.length - 1) {
        nextHouseAngle = houses.first.houseInitAngle ?? 0;
      } else {
  ...
  @override
  bool shouldRepaint(covariant _FDAstrolabeSpecialPainter oldDelegate) {
